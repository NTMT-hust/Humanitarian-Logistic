...
public class GoogleCseCollector extends Collector<SearchCriteria, OkHttpClient, List<SocialPost>> {
    ...
    private SocialPost parseSearchResult(JsonObject item, SearchCriteria criteria) {
        ...
        // ----------------------------------------------------------
        // FETCH FULL PAGE CONTENT (instead of only title + snippet)
        // ----------------------------------------------------------
        String fullText = fetchFullPageText(link);

        post.setContent(fullText != null ? fullText : "Content not available");
        ...
        metadata.put("snippet", fullText != null ? fullText : snippet);
    ...
    private String fetchFullPageText(String url) {
        try {
            ...
            return "Error fetching full content: " + e.getMessage();
        }
    }
